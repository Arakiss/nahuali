...
if current_version == version && current_identifier == expected_identifier
  puts "MCP server metadata synchronized at #{version}"
  exit 0
end

if ENV.fetch("NAHUALI_SYNC_MODE") == "check"
  warn "MCP server metadata is stale: version=#{current_version.inspect}, identifier=#{current_identifier.inspect}; expected #{version.inspect} and #{expected_identifier.inspect}"
  exit 1
end
